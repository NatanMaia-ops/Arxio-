...
export function OnboardingForm() {
	...
	if (loadingState === "loading") {
		return (
			<main
				className="flex min-h-dvh items-center justify-center bg-white px-4 py-8"
				role="status"
			>
	...
	if (loadingState === "error") {
		return (
			<main className="flex min-h-dvh items-center justify-center bg-white px-4 py-8 text-[#111]">
				<section className="w-full max-w-md rounded-2xl border border-[#e3e3e3] bg-white p-8 text-center">
					<h1 className={`${displayFont.className} font-bold text-3xl`}>
						Não foi possível carregar seu perfil
					</h1>
					<p className="mt-3 text-[#616161] text-[15px] leading-6">
						Verifique sua conexão e tente novamente.
					</p>
					<Button
						type="button"
						onClick={() => void loadOnboarding()}
						className="mt-6 h-11 rounded-full bg-black px-6 text-sm text-white hover:bg-[#292929]"
					>
						Tentar novamente
					</Button>
				</section>
	...
	const fieldClassName =
		"h-12 rounded-lg border border-[#d1d1d1] bg-white px-4 text-[15px] text-[#111] shadow-none focus-visible:border-[#111] focus-visible:ring-1 focus-visible:ring-[#111]/20 md:text-[15px]";
	const labelClassName =
		"text-[13px] font-medium leading-[18px] text-[#616161]";

	return (
		<main className="flex min-h-dvh items-center justify-center bg-white px-4 py-8 text-[#111] sm:px-6">
			<section className="w-full max-w-[820px] rounded-2xl border border-[#e3e3e3] bg-white px-6 py-8 sm:px-9 sm:py-10 lg:px-12 lg:py-11">
				<form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
					<p className="font-semibold text-[#616161] text-xs leading-4 tracking-[0.8px]">
						CONFIGURAÇÃO DO PERFIL
					</p>

					<h1
						className={`${displayFont.className} font-bold text-[34px] leading-[1.1] sm:text-[40px] sm:leading-11`}
					>
						Vamos montar seu perfil
					</h1>

					<p className="max-w-3xl text-[#616161] text-[15px] leading-6 sm:text-base">
					...
					<div className="flex flex-col gap-1.5">
						...
						<Input
							...
							readOnly
							aria-readonly="true"
							className={`${fieldClassName} cursor-default bg-[#f7f7f6] text-[#616161]`}
						/>
					</div>
					...
					<p className="text-[#616161] text-[13px] leading-[18px]">
					...
					<div className="mt-1 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
						<Button
							...
							disabled={isSubmitting}
							className="h-11 rounded-full border border-[#b8b8b8] bg-white px-6 font-medium text-[#111] text-[15px] shadow-[inset_0_0_0_1px_#b8b8b8] hover:border-[#858585] hover:bg-[#f7f7f6] hover:text-[#111] hover:shadow-[inset_0_0_0_1px_#858585] focus-visible:text-[#111]"
						>
						...
						<Button
							type="submit"
							disabled={isSubmitting}
							className="h-11 rounded-full bg-black px-7 font-medium text-[15px] text-white hover:bg-[#292929] sm:min-w-[230px]"
						>
							{submission === "complete"
								? "Salvando..."
